...
//--------------------------------------------------------------
// Определение исключительной ситуации, возникающей при попытке
// доступа к несуществующему элементу списка.
//--------------------------------------------------------------
...
protected:
  ...
  //--------------------------------------------------------------
  // Реализация внешнего итератора списка
  //--------------------------------------------------------------
  class ListIterator : public Iterator<T> {
    // Указатель на указатель на текущий элемент
    ListItem ** currentPtr;

  public :

    // Конструктор, использующийся в методе iterator списка
    ListIterator(List<T> * list) : currentPtr(&list->first) {}

    // Реализация операций итератора:

    bool hasMoreElements() const { return *currentPtr != NULL; }
    ...
    void remove() {
      ListItem * itemToDelete = *currentPtr;
      if (itemToDelete) {
        *currentPtr = itemToDelete->next;
        delete itemToDelete;
      }
    }
  };
