...
void * BinTwinsMemory::get(size_t sz) {
  ...
  demanded += sz;
  reserved += 1 << p;
  // Выдаем адрес блока, смещение которого было вычислено
...
void BinTwinsMemory::release(void * ptr, size_t sz) {
  cout << "----- Releasing " << sz << " bytes\n";
  // Вычисляем размерность p блока
  char p = 2;
  while (sz != (1 << p) && (~((1u << p)-1) & sz) != 0) p++;
  // Обращаемся к рекурсивной функции возврата блока в систему
  releaseRec((char*)ptr - buffer, p);
}
